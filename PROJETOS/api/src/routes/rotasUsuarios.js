...
router.put('/usuarios/:id_usuarios', async (req, res) => {
    ...
    try {
        ...
        const verificarUsuario = await BD.query(`SELECT * FROM usuarios WHERE id_usuarios = $1 and ativo = true`, [id_usuarios]);
        ...
        const comando = `UPDATE usuarios SET nome = $1, email = $2, senha = $3 WHERE, tipo_acesso = $4, WHERE  id_usuarios = $5`;
        const valores = [nome, email, senha, tipo_acesso, id_usuarios];
        await BD.query(comando, valores);
...
router.patch('/usuarios/:id_usuarios', async (req, res) => {
    ...
    try {
        ...
        valores.push(id_usuarios);

        //Monatndo a query dinamicamente
        const comando = `UPDATE usuarios SET ${campos.join(`, `)} WHERE id_usuarios = $${contador} `
        await BD.query(comando, valores);
...
router.delete('/usuarios/:id_usuarios', async (req, res) => {
    ...
    try {
        const comando = `UPDATE usuarios set ativo = false WHERE id_usuarios = $1`
        await BD.query(comando, [id_usuarios]);
